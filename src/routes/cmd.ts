...
export class CmdRouter {
  ...
  constructor(private producer: Producer) {
    this._router.post('/cmd/:type/:id', (req: Request, res: Response) => {
      ...
      const queueDef = new QueueDef(req.params.type, req.params.id, req.header('fiware-service'), req.header('fiware-servicepath'));
      this.producer.produce(queueDef, req.body)
        .then((deliveryId) => {
